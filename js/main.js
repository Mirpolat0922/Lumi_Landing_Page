 ...
 * LUMI — js/main.js
 *
 * Dynamic content (activities, packages) is driven by js/config.js.
 * To change directions or packages, edit LUMI_CONFIG.ACTIVITIES and
 * LUMI_CONFIG.PACKAGES in config.js — no changes needed here.
 */
...
const T = {
  uz: {
    ...
    f3h: "Ishonchli hamkorlar",    f3p: "Faqat tekshirilgan va sifatli markazlar bilan ishlaymiz. Ota-onalar izohlari to'g'ri tanlov qilishga yordam beradi.",
    ...
    q1:"Lumi nima?",                         a1:"Lumi — 0 dan 16 yoshgacha bo'lgan bolalar uchun Toshkentdagi 30+ hamkor markazni birlashtirgan platforma. Sport, ijod, ta'lim — hammasi bir joyda.",
    ...
    q4:"Tangalar tugab qoladimi?",           a4:"Yo'q. Lumi Tangalaringiz hisobingizda saqlanib qoladi va istagan vaqtda, istagan hamkor markazda ishlatishingiz mumkin.",
    ...
    q6:"Qanday yo'nalishlar mavjud?",        a6:"Ijod, ta'lim, sport, ko'ngilochar, rivojlantiruvchi, sog'liq, raqs, suzish, musiqa — 50+ yo'nalish, 30+ hamkor markaz.",
  ...
  en: {
    ...
    hfc_map_label:"Partner Centers", hfc_mgmt_month:"March",
    hfc_map_centers:"partner centers", hfc_mgmt_label:"My Bookings",
    hfc_mgmt_pending:"New booking", hfc_status_ok:"✓",
    ...
    s1v:"30+",s1d:"Partners", s2v:"50+",s2d:"Directions", s3v:"4",s3d:"Packages", s4v:"24/7",s4d:"Support",
    ...
    faq_p:"Got a question? We've got the answer. For more, reach out via Telegram.",
    q1:"What is Lumi?",               a1:"Lumi is a platform that connects families to 30+ verified children's activity centers in Tashkent. Sports, arts, education — all in one place.",
    q2:"Is there a subscription?",    a2:"No. With the Lumi Coins system you only pay for sessions you actually use. You can freely choose between multiple centers.",
    ...
    q4:"Do unused coins expire?",     a4:"No. Your Lumi Coins stay in your account and can be used at any time at any partner center.",
    ...
    q6:"What directions are available?",a6:"Creativity, education, sports, entertainment, developmental, health, dance, swimming, music — 50+ activities across 30+ partner centers.",
    ...
    cta_note:"", cta_webapp:"Start Now →", cta_tg:"Via Telegram", cta_sub:"Free · No registration required",
  ...
  ru: {
    ...
    hfc_map_label:"Центры партнёров", hfc_mgmt_month:"Март",
    hfc_map_centers:"партнёрских центров", hfc_mgmt_label:"Мои брони",
    hfc_mgmt_pending:"Новая запись", hfc_status_ok:"✓",
    ...
    f3h:"Проверенные партнёры",    f3p:"Работаем только с проверенными центрами. Отзывы помогут сделать правильный выбор.",
    f4h:"Отслеживайте прогресс",   f4p:"Следите за достижениями ребёнка и получайте рекомендации.",
    f5h:"Экономьте деньги",        f5p:"С Lumi Coins вы платите только за использованные занятия. Никаких лишних абонементов.",
    ...
    s1v:"30+",s1d:"Партнёры", s2v:"50+",s2d:"Направлений", s3v:"4",s3d:"Пакетов", s4v:"24/7",s4d:"Поддержка",
    ...
    faq_p:"Есть вопрос? У нас есть ответ. По другим вопросам пишите в Telegram.",
    ...
    q3:"Как работают Lumi Coins?",     a3:"Lumi Coins — цифровые единицы для записи в центры. Каждый центр устанавливает свою цену в монетах, что даёт вам свободу выбора.",
    q4:"Сгорают ли неиспользованные монеты?", a4:"Нет. Ваши Lumi Coins остаются на счёте и могут быть использованы в любое время в любом центре-партнёре.",
    ...
    q6:"Какие направления доступны?",  a6:"Творчество, образование, спорт, развлечения, развивающие, здоровье, танцы, плавание, музыка — 50+ направлений в 30+ центрах.",
    ...
    cta_note:"", cta_webapp:"Начать сейчас →", cta_tg:"Через Telegram", cta_sub:"Бесплатно · Без регистрации",
...
function setLang(lang) {
  ...
  // Rebuild dynamic multilingual content
  buildHeroCards();
  buildActivities();
  buildPricingCards();
...
function trackWebApp(location)    { ymEvent("cta_click", { cta_type: "web_app",      cta_location: location }); }
function trackTelegram(location)  { ymEvent("cta_click", { cta_type: "telegram",     cta_location: location }); }
function trackPartnerCTA(location){ ymEvent("cta_click", { cta_type: "partner_form", cta_location: location }); }
function trackPricing(planName)   { ymEvent("pricing_click", { plan_name: planName }); }
...
/* ═══════════════════════════════════════════════════════════════
   ...
   Built from LUMI_CONFIG.ACTIVITIES. To change content or add
   images, edit the ACTIVITIES array in js/config.js.
   ═══════════════════════════════════════════════════════════════ */
function buildActivities() {
  if (typeof LUMI_CONFIG === "undefined") return;
  const grid = document.getElementById("activitiesGrid");
  if (!grid) return;
  const activities = LUMI_CONFIG.ACTIVITIES;
  ...
  grid.innerHTML = activities.map((act, i) => {
    ...
    const name  = act.name[currentLang] || act.name.uz;
    const desc  = act.desc[currentLang] || act.desc.uz;
    const imgEl = act.image
      ? `<img src="${act.image}" alt="${name}" class="activity-photo" loading="lazy" />`
      : `<img src="" alt="${name}" class="activity-photo" loading="lazy" />`;

    return `
      <article class="activity-card reveal ${delay}">
        ${imgEl}
        <div class="activity-body">
          <h4>${name}</h4>
          <p>${desc}</p>
          <a href="#"
             class="activity-cta-link"
             onclick="trackTelegram('activities'); this.href=LUMI_CONFIG.TELEGRAM_BOT_URL; return true;"
             target="_blank" rel="noopener"
             data-i18n="act_book">${T[currentLang].act_book}</a>
        </div>
      </article>`;
  }).join("");

  // Re-attach IntersectionObserver for newly created .reveal elements
...
/* ═══════════════════════════════════════════════════════════════
   PRICING CARDS
   Built from LUMI_CONFIG.PACKAGES. To change packages, prices
   or coin amounts, edit the PACKAGES array in js/config.js.
   ═══════════════════════════════════════════════════════════════ */
function buildPricingCards() {
  if (typeof LUMI_CONFIG === "undefined") return;
  const grid = document.getElementById("pricingGrid");
  if (!grid) return;
  const packages = LUMI_CONFIG.PACKAGES;
  if (!packages || !packages.length) return;

  const t = T[currentLang];
  const delays = ["d1", "d2", "d3", "d1"];

  grid.innerHTML = packages.map((pkg, i) => {
    const delay    = delays[i % 4];
    const name     = pkg.name[currentLang] || pkg.name.uz;
    const featured = pkg.featured;

    return `
      <div class="pricing-card${featured ? " featured" : ""} reveal ${delay}">
        ${featured ? `<span class="feat-badge" data-i18n="popular">${t.popular}</span>` : ""}
        <div class="plan-name">${name}</div>
        <div class="plan-price">
          <div class="plan-amount">${pkg.price}</div>
          <div class="plan-currency">UZS</div>
        </div>
        <div class="coin-info">
          <img src="assets/logos/logo-coin.png" alt="coin" class="ci-img" />
          <span class="ci-count">${pkg.coins}</span>
          <span class="ci-text" data-i18n="coin_label">${t.coin_label}</span>
        </div>
        <a href="#"
           class="plan-btn"
           onclick="trackPricing('${name}'); trackTelegram('pricing'); this.href=LUMI_CONFIG.TELEGRAM_BOT_URL; return true;"
           target="_blank" rel="noopener"
           data-i18n="plan_btn">${t.plan_btn}</a>
      </div>`;
  }).join("");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Build dynamic grids first so their elements exist for the observer
  buildActivities();
  buildPricingCards();
  ...
  initRevealObserver();     // attach observer after all DOM is ready
  initNavbar();
  buildPartnersStrip();
  buildFooterLinks();
